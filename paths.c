...
/**
 * path_finder - Acts as an interface to
...
/**
 * find_path - Finds the index of an environmental var.
 * @str: Env variable that needs to be found.
 * Return: returns the index of the environmental variable.
 * otherwise returns -1.
 */
...
/**
 * tokenize_path - Separates a string representing paths to an array
...
/**
 * search_directories - search through directories stored in path_tokens for a
...
char *search_directories(char **path_tokens, char *command)
{
	...
	{
		d = chdir(path_tokens[i]);
		if (d == -1)
		{
			perror("ERROR!");
			return (NULL);
		}
		d = stat(command, &stat_buf);
		if (d == 0)
		{
			chdir(cwd);
			free(cwd);
...
/**
 * build_path - Combines two strings path directory and
